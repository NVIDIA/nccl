...
```mermaid
flowchart TD
    A["ncclTopoGetSystem<br/>src/graph/topo.cc"] --> B["ncclTopoComputePaths<br/>src/graph/paths.cc"]
    B --> C["ncclTopoCompute<br/>src/graph/search.cc"]
    C --> D["ncclTopoPreset / Postset<br/>src/graph/connect.cc"]
    D --> E["selectTransport<br/>src/transport.cc"]
    E --> F["ncclTopoTuneModel<br/>src/graph/tuning.cc"]
    F --> G["cost table consumed by enqueue.cc"]
```

> Seeing it in logs: every stage below is observable with `NCCL_DEBUG=INFO` plus
> a `NCCL_DEBUG_SUBSYS` filter. Topology, paths, and graph search print under
> `GRAPH`; transport connection prints under `INIT` (with `NET`/`P2P`/`SHM` for
> transport detail); the tuning model prints under `TUNING`.
...
To inspect the discovered machine graph, run with
...
can compare. These path classes are also printed under `NCCL_DEBUG_SUBSYS=GRAPH`.
...
communication-graph search tailored to collective algorithms. The searched
graphs are logged under `NCCL_DEBUG_SUBSYS=GRAPH`.
...
The resulting per-rank channel setup is visible under `NCCL_DEBUG_SUBSYS=INIT`.
...
The order in `src/transport.cc` matters because `selectTransport(...)` iterates
the transport list and stops at the first one whose `canConnect` says "yes".
Two details are worth clarifying:

- It only iterates the `NTRANSPORTS` real transports (P2P, SHM, NET, CollNet). The
  profiler transport (`TRANSPORT_PROFILER`) sits past `NTRANSPORTS` and is never
  asked here.
- The CollNet transport's `canConnect` always answers "no" for a normal
  point-to-point edge (CollNet is set up through a separate, collective-specific
  path). So in practice the per-edge winners are P2P, SHM, or NET.

You can watch this with `NCCL_DEBUG=INFO NCCL_DEBUG_SUBSYS=INIT` (plus
...
when a real collective arrives. The generated tables are printed under
