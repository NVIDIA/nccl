...
__device__ __forceinline__ void ncclSymRun_AllReduce_AGxLL_R_impl(ncclSymDevArgs const* args) {
  ncclSymPrims prim(args->comm);
  int /*const&*/ rank = prim.rank;
  ...
  while (0 < nElts) {
    if (t*EltPerPack < nElts) {
      ...
      prim.template bcastLL<multimem>(/*slot=*/nIterPacks*rank + t, inp);
...
__device__ __forceinline__ void ncclSymRun_AllReduce_AGxLL_R(ncclSymDevArgs const* args) {
  ncclSymRun_AllReduce_AGxLL_R_impl<Red, T, /*multimem=*/false>(args);
}
template<template<typename> typename Red, typename T>
__device__ __forceinline__ void ncclSymRun_AllReduce_AGxLLMC_R(ncclSymDevArgs const* args) {
  ncclSymRun_AllReduce_AGxLL_R_impl<Red, T, /*multimem=*/true>(args);
}
