...
__device__ __forceinline__ void red_release_sys_global_add_u64(uint64_t* addr, size_t val) {
  asm volatile(
...
__global__ void allGatherCftCountedKernel(ncclWindow_t sendWin, ncclWindow_t recvWin, size_t count,
                                          ncclDevComm devComm) {
  ...
  const int globalTid = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  for (size_t offset = globalTid; offset < count; offset += globalNthreads) {
    recv[rank * count + offset] = send[offset];
    size_t chunkElems = (count - offset) < (size_t)blockDim.x ? (count - offset) : (size_t)blockDim.x;
    ctaBytes += chunkElems * sizeof(float);
  }
  ...
  coop.sync();
  if (coop.thread_rank() == 0) red_release_sys_global_add_u64(counterPtr, ctaBytes);
  if (nRanks == 1) return;
  ...
  for (size_t base = blockIdx.x * blockDim.x; base < count; base += globalNthreads) {
    ...
    for (int peer = 0; peer < nRanks; peer++) {
      ...
      ncclGetCftLeInfo(recvWin, rank * count * sizeof(float), peer, team, devComm, &leId, &leOffset);
      size_t dataOffset = leOffset + base * sizeof(float);
      size_t counterOffset = ALIGN_UP(leOffset + ((nRanks - rank) * count * sizeof(float)), 256u);
      cft.putCounted(coop, leId, dataOffset, counterOffset, smem, chunkBytes);
    }
    cft.submit(coop);
    cft.flush(coop);
  }
  cft.waitCounted(coop, cuda::memory_order_acquire, ncclMemProxyType::Generic, counterPtr, bytes*nRanks, devComm.abortFlag);
#else
  (void)sendWin;
  (void)recvWin;
  (void)count;
...
void* allGatherCftCounted(int myRank, int totalRanks, int localDevice, int devicesPerRank) {
  (void)devicesPerRank;
  ncclComm_t comm;
  ncclUniqueId id;
  ...
  printf("rank %d allgather_cft_counted: SKIPPED, requires CUDA Toolkit 13.3 or newer\n", myRank);
  return NULL;
#endif
  cudaDeviceProp prop;
  CUDACHECK(cudaGetDeviceProperties(&prop, localDevice));
  if (prop.major * 10 + prop.minor < 100) {
    printf("rank %d allgather_cft_counted: SKIPPED, requires compute capability 10.0 or newer\n", myRank);
    return NULL;
  }
  ...
  allGatherCftCountedKernel<<<NCCL_DEVICE_CTA_COUNT, NCCL_DEVICE_THREADS_PER_CTA,
...
done_check:
  printf("rank %d allgather_cft_counted: %s\n", myRank, ok ? "PASSED" : "FAILED");
...
int main(int argc, char* argv[]) {
  return run_example(argc, argv, allGatherCftCounted);
}
