...
static void ncclDebugInit()
{
  ...
  {
    if (ncclDebugTimestampFormat[i] == '_')
      ncclDebugTimestampFormat[i] = ' ';
  }
...
void ncclDebugLog(ncclDebugLogLevel level, unsigned long flags, const char *filefunc, int line, const char *fmt, ...)
{
  ...
  if (ncclDebugTimestampLevels & (1 << level))
  {
    if (ncclDebugTimestampFormat[0] != '\0')
    {
      ...
      // Convert to Unix epoch (100-nanosecond intervals since Jan 1, 1601)
      uint64_t unixTime = (uli.QuadPart - 116444736000000000ULL);
      time_t sec = unixTime / 10000000ULL;
      long subsec_nsec = (unixTime % 10000000ULL) * 100;
      std::tm nowTm;
      ...
      if (ncclDebugTimestampSubsecondsStart != -1)
      {
        ...
        snprintf(localTimestampFormat + ncclDebugTimestampSubsecondsStart,
                 ...
                 subsec_nsec / (1000000000UL / ncclDebugTimestampMaxSubseconds));
...
__attribute__((alias("ncclResetDebugInit"))) void
pncclResetDebugInit();
extern "C"
    __attribute__((visibility("default")))
    __attribute__((weak))
    __attribute__((deprecated("ncclResetDebugInit is not supported as part of the NCCL API and will be removed in the future"))) void
    ncclResetDebugInit();
