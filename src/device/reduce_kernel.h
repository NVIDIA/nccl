...
__device__ __forceinline__ Pack applyReduce(Fn fn, Pack a, Pack b)
{
  return fromPack<Pack>(
      Apply_Reduce_MaybeEmpty<Fn, BytePackOf<Pack>::Size / sizeof(typename Fn::EltType)>::reduce(fn, toPack(a), toPack(b)));
}
...
// FP8 types need explicit static_cast because float2/float4 don't have constructors
// that take FP8 vector types, but FP8 types have conversion operators to float2/float4
#if defined(__CUDA_FP8_TYPES_EXIST__)
#define FP8_CAST(A, B, EltPerPack, VecA, VecB)                                                                  \
  template <>                                                                                                   \
  struct Apply_Cast<A, B, EltPerPack>                                                                           \
  {                                                                                                             \
    __device__ __forceinline__ static BytePack<sizeof(B) * EltPerPack> cast(BytePack<sizeof(A) * EltPerPack> a) \
...
FP8_CAST(__nv_fp8_e5m2, float, 2, __nv_fp8x2_e5m2, float2)
FP8_CAST(__nv_fp8_e5m2, float, 4, __nv_fp8x4_e5m2, float4)

FP8_CAST(__nv_fp8_e4m3, float, 2, __nv_fp8x2_e4m3, float2)
FP8_CAST(__nv_fp8_e4m3, float, 4, __nv_fp8x4_e4m3, float4)
#undef FP8_CAST
#endif
#undef EASY_CAST
...
struct LoadMultimem_BigPackSize
{
  ...
  static constexpr int BigPackSize =
      IsFloat && IsSum && sizeof(T) < 8 ? 16 : IsFloat && IsSum                                ? sizeof(T)
