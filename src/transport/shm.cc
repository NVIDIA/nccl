...
static ncclResult_t shmCanConnect(int *ret, struct ncclComm *comm, struct ncclTopoGraph *graph, struct ncclPeerInfo *info1, struct ncclPeerInfo *info2)
{
  *ret = 0;
  ...
  // SHM transport relies on POSIX shared memory which is not available on Windows.
  // Fall back to NET transport instead.
  return ncclSuccess;
#endif
...
static ncclResult_t shmSendProxyConnect(struct ncclProxyConnection *connection, struct ncclProxyState *proxyState, void *reqBuff, int reqSize, void *respBuff, int respSize, int *done)
{
  ...
  if (reqSize != sizeof(struct shmProxyInfo) || respSize != sizeof(struct shmProxyInfo))
  ...
  if (respSize != sizeof(struct shmProxyInfo))
    return ncclInternalError;
...
fail:
  if (proxyInfo->ceRecvMem)
    ncclCudaHostFree(proxyInfo->ceRecvMem);
  if (proxyInfo->devFifo)
    (void)ncclCudaFree(proxyInfo->devFifo);
  free(proxyInfo);
  goto exit;
}
...
static ncclResult_t shmSendProxySetup(struct ncclProxyConnection *connection, struct ncclProxyState *proxyState, void *reqBuff, int reqSize, void *respBuff, int respSize, int *done)
{
  ...
  /* check message size */
  if (reqSize != sizeof(struct shmRequest))
    return ncclInternalError;
  if (respSize != sizeof(struct shmConnectInfo))
    return ncclInternalError;
