...
#include <type_traits>
#include "device.h" // For nccl_ulong2_16
...
union alignas(16) BytePack<16>
{
  ...
  uint32_t u32[4];
  uint64_t u64[2];
  nccl_ulong2_16 ul2[1], native; // Use 16-byte type on Windows
};
template <int Size>
union BytePack
{
...
template <>
__device__ __forceinline__ BytePack<0> ld_global<0>(uintptr_t addr) { return {}; }
template <>
__device__ __forceinline__ BytePack<0> ld_shared<0>(uint32_t addr) { return {}; }
template <>
__device__ __forceinline__ BytePack<0> ld_volatile_global<0>(uintptr_t addr) { return {}; }
template <>
__device__ __forceinline__ BytePack<0> ld_volatile_shared<0>(uint32_t addr) { return {}; }
template <>
__device__ __forceinline__ BytePack<0> ld_relaxed_gpu_global<0>(uintptr_t addr) { return {}; }
template <>
...
// Overload for signed 64-bit (ssize_t/int64_t) - needed for connFifo.size writes visible to CPU proxy
// Note: On Windows SSIZE_T and int64_t are the same type in CUDA/nvcc
__device__ __forceinline__ void st_relaxed_sys_global(int64_t *ptr, int64_t val)
{
#if __CUDA_ARCH__ >= 700
  asm volatile("st.relaxed.sys.global.s64 [%0], %1;" ::"l"(cvta_to_global(ptr)), "l"(val) : "memory");
#else
  asm volatile("st.volatile.global.s64 [%0], %1;" ::"l"(cvta_to_global(ptr)), "l"(val) : "memory");
#endif
}
// Overload for 32-bit signed int
__device__ __forceinline__ void st_relaxed_sys_global(int32_t *ptr, int32_t val)
{
#if __CUDA_ARCH__ >= 700
  asm volatile("st.relaxed.sys.global.s32 [%0], %1;" ::"l"(cvta_to_global(ptr)), "r"(val) : "memory");
#else
  asm volatile("st.volatile.global.s32 [%0], %1;" ::"l"(cvta_to_global(ptr)), "r"(val) : "memory");
#endif
}
...
{
  asm volatile("multimem.st.global.v4.f32 [%0], {%1,%2,%3,%4};" ::"l"(addr), "r"(val.u32[0]), "r"(val.u32[1]), "r"(val.u32[2]), "r"(val.u32[3])
               : "memory");
}
#else
template <int Size>
...
#pragma unroll
    for (i = 0; i < Size / 4; i++)
    {
      if (i * 4 / sizeof(T) < 1 || i * 4 / sizeof(T) < n)
        part[i] = down[i];
    }
    uint32_t extra;
    if (misalign)
...
#pragma unroll
    ...
    if (misalign)
      part[i] = __funnelshift_r(part[i], extra, 8 * misalign);
    return ans;
  }
  else
  {
...
#pragma unroll
    ...
    {
      if (i < 1 || i < n)
        part[i] = ((BytePack<sizeof(T)> *)ptr)[i];
    }
    return ans;
  }
}
...
#pragma unroll
  ...
  {
    if (i < 1 || i < n)
      ((BytePack<sizeof(T)> *)ptr)[i] = part[i];
  }
}
