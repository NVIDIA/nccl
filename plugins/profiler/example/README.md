...
- `NCCL_PROFILE_KERNEL_STEP_POOL_SIZE` (4096) — per-slice Simple-prims `KernelStep` events
...
## KernelStep events (intra-host Simple prims)

`ncclProfileKernelStep` (`1 << 15` = 32768) records GPU `globaltimer` start/stop for each
Simple-protocol slice on the Wait/Post role threads (PCIe / NVLink / SHM paths that use
`prims_simple.h`). Example:

```
NCCL_PROFILE_EVENT_MASK=32774   # Coll|P2p|KernelStep
NCCL_PROFILE_DUMP_FILE=/tmp/nccl-trace
NCCL_PROTO=Simple
```

Chrome-trace entries are named `"KernelStep"` with args `Channel`, `IsSend`, `Peer`, `Step`,
`Size`, `StartGpuClk`, `StopGpuClk`.
