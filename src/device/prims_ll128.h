...
class Primitives<T, RedOp, Fan, Direct, ProtoLL128, P2p, isNetOffload> : public PrimitivesWithoutDirect<Primitives<T, RedOp, Fan, Direct, ProtoLL128, P2p, isNetOffload>>
{
  ...
  inline __device__ void waitSend(int nbytes)
  {
    if (sendConnHeadPtr)
    {
      ...
      if (sendConnFifo)
      {
        ...
        st_relaxed_sys_global((int64_t *)&sendConnFifo[sendStep[wid] % NCCL_STEPS].size, (int64_t)nbytes);
      }
      sendConnHead += 1;
    }
  }
...
#pragma unroll
  ...
  __device__ __forceinline__ void GenericOp(intptr_t srcIx, intptr_t dstIx, int nelem, bool postOp)
  {
    ...
    if (SEND)
      waitSend(divUp(nelem, DataEltPerSlice) * WireWordPerSlice * sizeof(uint64_t));
    barrier();
