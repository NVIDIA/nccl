...
static ncclResult_t gdakiConnectQp(struct gdaki_context *ctx, struct doca_gpu_verbs_qp_hl *gqp,
  ...
  DOCACHECKGOTO(doca_verbs_qp_attr_set_ack_timeout(verbs_qp_attr, 20), docaStatus, status,
                destroy_verbs_qp_attr);
  DOCACHECKGOTO(doca_verbs_qp_attr_set_retry_cnt(verbs_qp_attr, 7), docaStatus, status,
                destroy_verbs_qp_attr);
...
ncclResult_t ncclGinGdakiCreateContext(void *collComm, int nSignals, int nCounters,
  ...
  const int ib_gid_index = (ncclParamIbGidIndex() != -1) ? ncclParamIbGidIndex() : 0;
