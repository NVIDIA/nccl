  ...
  *closed = 0;
  ...
  do
  {
    ...
    if (bytes == SOCKET_ERROR)
    {
      ...
      if ((op == NCCL_SOCKET_SEND && wsaErr == WSAENOTCONN) || (op == NCCL_SOCKET_RECV && wsaErr == WSAECONNRESET))
      {
        *closed = 1;
        return ncclSuccess;
      }
      ...
      {
        WARN("socketProgressOpt: Call to %s %s failed : error %d", (op == NCCL_SOCKET_RECV ? "recv from" : "send to"),
...
#endif
    (*offset) += bytes;
    if (sock->abortFlag && NCCL_ATOMIC_LOAD(sock->abortFlag))
    {
                                ...
                                int *nIfs)
{
  ...
  *nIfs = 0;
  ...
  else
  {
    ...
    if (*nIfs == 0)
      NCCLCHECK(findInterfaces("^docker,lo,virbr", ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs, nIfs));
    // Finally look for docker, then lo.
    if (*nIfs == 0)
      NCCLCHECK(findInterfaces("docker", ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs, nIfs));
    if (*nIfs == 0)
      NCCLCHECK(findInterfaces("lo", ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs, nIfs));
    if (*nIfs == 0)
      NCCLCHECK(findInterfaces("virbr", ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs, nIfs));
  }
  return ncclSuccess;
}
...
static ncclResult_t socketSetFlags(struct ncclSocket *sock)
{
  ...
  if (sndBuf > 0)
    SYSCHECK(setsockopt(sock->fd, SOL_SOCKET, SO_SNDBUF, (char *)&sndBuf, sizeof(int)), "setsockopt SO_SNDBUF");
  if (rcvBuf > 0)
    SYSCHECK(setsockopt(sock->fd, SOL_SOCKET, SO_RCVBUF, (char *)&rcvBuf, sizeof(int)), "setsockopt SO_RCVBUF");
  return ncclSuccess;
}
...
ncclResult_t ncclSocketSendRecv(struct ncclSocket *sendSock, void *sendPtr, int sendSize, struct ncclSocket *recvSock, void *recvPtr, int recvSize)
{
  ...
  while (sendOffset < sendSize || recvOffset < recvSize)
  {
    if (sendOffset < sendSize)
      NCCLCHECK(socketProgress(NCCL_SOCKET_SEND, sendSock, sendPtr, sendSize, &sendOffset));
    if (recvOffset < recvSize)
      NCCLCHECK(socketProgress(NCCL_SOCKET_RECV, recvSock, recvPtr, recvSize, &recvOffset));
  }
  return ncclSuccess;
}
  ...
  if (sock != NULL)
  {
    if (sock->state > ncclSocketStateNone && sock->state < ncclSocketStateNum && sock->fd >= 0)
    {
      if (wait)
      {
        ...
        {
          int offset = 0;
          if (ncclSocketProgress(NCCL_SOCKET_RECV, sock, &data, sizeof(char), &offset, &closed) != ncclSuccess)
            break;
        } while (closed == 0);
      }
