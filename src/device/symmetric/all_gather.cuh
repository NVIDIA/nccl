...
static __device__ void bcastMultimem(
  ...
  if ((inputUptr-outputUptr)%16 == 0) {
    ...
    #if 0
      // This loop produces SASS like:
      //   ST.E.128 r0
      //   LD.E.128 r0
      //   ST.E.128 r1
      //   LD.E.128 r1
      // The benefit is that as soon as a register is free to reuse (after the store)
      // we immediatley issue a load to it that at no point is this warp underutilized.
      if (0 < nIters) {
        uintptr_t cursorPrev = cursor;
        ...
        while (0 < nIters) {
          #pragma unroll
          for (int u=0; u < UnrollPacks; u++) {
            multimem_st_global(outputUptr + cursorPrev + u*WARP_SIZE*BytePerPack, tmp[u]);
            tmp[u] = *reinterpret_cast<BytePack<BytePerPack>*>(inputUptr + cursor + u*WARP_SIZE*BytePerPack);
          }
          cursorPrev = cursor;
          cursor += tn*UnrollPacks*BytePerPack;
        ...
        cursorPrev += tn*UnrollPacks*BytePerPack;
      }
    #else
      // Look which produces an unrolled blob of LD.E.128 followed by blob of ST.E.128
      #pragma unroll 1
      while (0 < nIters) {
        BytePack<BytePerPack> tmp[UnrollPacks];
        #pragma unroll
...
__device__ __forceinline__ void ncclSymRun_AllGather_STMC(ncclSymDevArgs const* args) {
  ...
  #if 0 // Partition memory to blocks
    int b = prim.block;
    int bn = prim.nBlocks;
    int t = threadIdx.x;
    int tn = blockDim.x;
    size_t partBytes = idivFast64(bytes, bn, idivRcp64_upto64(bn));
    input += b*partBytes;
    output += b*partBytes;
    partBytes = min(partBytes, bytes - b*partBytes);
  #else // Round robin memory to blocks
    // Threads numbered over rank.
    int t = flattenIx(threadIdx.x%WARP_SIZE, WARP_SIZE,
                      prim.block, prim.nBlocks,
                      threadIdx.x/WARP_SIZE, blockDim.x/WARP_SIZE);
    int tn = prim.nBlocks*blockDim.x;
    size_t partBytes = bytes;
  #endif

  prim.barrierArrive(ncclCoopCta(), /*release=*/false);
  prim.barrierWait(ncclCoopCta(), /*acquire=*/false);

  bcastMultimem(prim, tn, t, input, output + rank*bytes, partBytes);

  prim.barrierArrive(ncclCoopCta(), /*release=*/true);
  prim.barrierWait(ncclCoopCta(), /*acquire=*/false);
}

template<bool multimem, typename EltType>
static __device__ void allgather_LL_body(
  ...
  while (0 < nElts) {
    ...
    if (t < nIterPacks) {
      ...
      prim.template bcastLL<multimem>(/*slot=*/nIterPacks*rank + t, x);
    }
...
static __device__ void ncclSymRun_AllGather_LL_impl(ncclSymDevArgs const* args) {
  ncclSymPrims prim(args->comm);
  using Pack = BytePack<8>;
  ...
  if (__builtin_expect(lowBits%8 == 0, true)) {
    ...
    allgather_LL_body<multimem>(prim, (BytePack<8>*)blockInput, (BytePack<8>*)blockOutput, nBlockElts/8, nBlockPacks, nElts/8);
  } else {
    allgather_LL_body<multimem>(prim, blockInput, blockOutput, nBlockElts, nBlockPacks, nElts);
  }
}

__device__ __forceinline__ void ncclSymRun_AllGather_LL(ncclSymDevArgs const* args) {
  ncclSymRun_AllGather_LL_impl</*multimem=*/false>(args);
}
__device__ __forceinline__ void ncclSymRun_AllGather_LLMC(ncclSymDevArgs const* args) {
  ncclSymRun_AllGather_LL_impl</*multimem=*/true>(args);
}
