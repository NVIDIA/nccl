...
#elif defined(_M_X64)
#include <intrin.h>
// MSVC uses different cpuid intrinsic signature
...
ncclResult_t xmlLoadSub(FILE *file, struct ncclXml *xml, struct ncclXmlNode *head, struct xmlHandler handlers[], int nHandlers)
{
  ...
  while (1)
  {
    ...
    {
      if (nHandlers)
        INFO(NCCL_GRAPH, "Ignoring element %s", node->name);
...
#ifndef _WIN32
/* Linux-specific sysfs directory traversal code */
...
static ncclResult_t getBcmLinks(const char *busId, int *nlinks, char **peers)
{
  ...
  *peers = NULL;
  ...
  if (dir)
  {
    ...
    {
      if (strlen(file->d_name) != BUSID_SIZE - 1)
        continue;
      char *path;
      if (getPciPath(file->d_name, &path) == ncclSystemError)
        continue;
      free(path);
...
#else
/* Windows stub - BCM links not available */
static ncclResult_t getPciPath(const char *busId, char **path)
{
  (void)busId;
  *path = NULL;
...
ncclResult_t ncclTopoGetXmlFromSys(struct ncclXmlNode *pciNode, struct ncclXml *xml)
{
  ...
  {
    if (path)
      NOWARN(ncclTopoSetAttrFromSys(pciNode, path, "vendor", "vendor"), NCCL_GRAPH);
  }
  NCCLCHECKNOWARN(xmlGetAttrIndex(pciNode, "device", &index), NCCL_GRAPH);
  if (index == -1)
  {
    if (path)
      NOWARN(ncclTopoSetAttrFromSys(pciNode, path, "device", "device"), NCCL_GRAPH);
  }
  NCCLCHECKNOWARN(xmlGetAttrIndex(pciNode, "subsystem_vendor", &index), NCCL_GRAPH);
  if (index == -1)
  {
    if (path)
      NOWARN(ncclTopoSetAttrFromSys(pciNode, path, "subsystem_vendor", "subsystem_vendor"), NCCL_GRAPH);
  }
  NCCLCHECKNOWARN(xmlGetAttrIndex(pciNode, "subsystem_device", &index), NCCL_GRAPH);
  if (index == -1)
  {
    if (path)
      NOWARN(ncclTopoSetAttrFromSys(pciNode, path, "subsystem_device", "subsystem_device"), NCCL_GRAPH);
  }
...
ncclResult_t ncclTopoGetXmlFromGpu(struct ncclXmlNode *pciNode, nvmlDevice_t nvmlDev, struct ncclXml *xml, struct ncclXmlNode **gpuNodeRet)
{
  ...
  if (gpuNode == NULL)
    NCCLCHECK(xmlAddNode(xml, pciNode, "gpu", &gpuNode));
  ...
  if (nvlNode == NULL)
  {
    ...
    int maxNvLinks = (sm < 60) ? 0 : (sm < 70) ? 4
                                 : (sm < 80)   ? 6
                                 : (sm < 90)   ? 12
                                               : 18;
...
ncclResult_t ncclTopoXmlGraphLoadGraphs(FILE *file, struct ncclXml *xmlGraph, struct ncclXmlNode *head)
{
  ...
  NCCLCHECK(xmlGetAttr(head, "name", &name));
  if (name != NULL)
    INFO(NCCL_GRAPH, "Loading graphs for topology %s", name);
  else
    INFO(NCCL_GRAPH, "Loading graphs");
