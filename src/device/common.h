...
#include "device_table_size.h"

#define COLL_UNROLL (ncclCollUnroll())

#if __CUDA_ARCH__ >= 700
// __grid_constant__ appears to break cuda-gdb
// Try enabling it to see if it fixes Windows parameter passing issue
...
extern __device__ ncclDevFuncPtr_t const ncclDevFuncTable[NCCL_DEV_FUNC_TABLE_SIZE];
...
#if __CUDA_ARCH__ >= 700
extern __shared__ nccl_ulong2_16 ncclShmemPerWarp[/*ncclShmemDynamicSize()/sizeof(nccl_ulong2_16)*/];
#else
extern __shared__ nccl_ulong2_16 ncclShmemPerWarp[ncclShmemScratchWarpSize() * (NCCL_MAX_NTHREADS / WARP_SIZE) / sizeof(nccl_ulong2_16)];
#endif
    ...
    int tid, int tn, struct ncclDevKernelArgs const *args, int batchIx)
{
  ...
  while (true)
  {
    ...
    if (tid < nPacks)
    {
      ...
      nccl_ulong2_16 tmp;  // Use 16-byte type (ulonglong2 on Windows, ulong2 on Linux)
      ...
      if (ncclShmem.args.workStorageType == ncclDevWorkStorageTypeArgs)
      {
        ...
        int byteOffset = batch.offsetBase + srcWork * workSize + packInWork * 16;
        // Calculate the index in the storage array (nccl_ulong2_16 is 16 bytes)
        int storageIdx = byteOffset / sizeof(nccl_ulong2_16);
        // Cast args to the 4K storage type to access the storage union member
        ncclDevKernelArgs4K const *args4K = (ncclDevKernelArgs4K const *)args;
...
{
  return (ncclShmem.workType == ncclDevWorkTypeP2p) ? ((struct ncclDevWorkP2p *)ncclShmem.workStorage)[workItemIdx].profilerEnabled : ((struct ncclDevWorkColl *)ncclShmem.workStorage)[workItemIdx].profilerEnabled;
}
...
__device__ __forceinline__ void profiler(int action)
{
  if (threadIdx.x == 0)
  {
    ...
    else
    {
      ...
      if (action == FINI)
        ((ncclKernelCommAndChannels *)ncclShmem.args.comm)->channels[ncclShmem.channelId].workCounter = ncclShmem.channel.workCounter;
    }
  }
}
