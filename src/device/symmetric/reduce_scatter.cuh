...
__device__ __forceinline__ void ncclSymRun_ReduceScatter_LD(ncclSymDevArgs const* args) {
  ...
  reduce(prim, btn, bt, /*waitNeeded=*/true, red, (T*)args->input + prim.rank*args->nElts, (T*)args->output, args->nElts);
...
__device__ __forceinline__ void ncclSymRun_ReduceScatter_LDMC(ncclSymDevArgs const* args) {
  ...
  int bt = flattenIx(threadIdx.x%WARP_SIZE, WARP_SIZE,
                     prim.block, prim.nBlocks,
                     threadIdx.x/WARP_SIZE, blockDim.x/WARP_SIZE);
      bt = flattenIx(threadIdx.x, blockDim.x,
                     prim.block, prim.nBlocks);
  int btn = prim.nBlocks*blockDim.x;
  ...
  reduceMultimem(prim, btn, bt, red, (T*)args->input + prim.rank*args->nElts, (T*)args->output, args->nElts);
...
__device__ __forceinline__ void ncclSymRun_ReduceScatter_LL_body(
  ...
  while (0 < nElts) {
    ...
    #if 0
      // NOTE: Unrolling seems to slightly hurt on sm90 even though it matches perfectly with what compiling for static nranks would produce.
      constexpr int Unroll = 8;
      #pragma unroll 1
      for (int i = t; i < (nRanks*nIterPacks & -(Unroll*tn)); i += Unroll*tn) {
        int peers[Unroll], packs[Unroll];
        #pragma unroll
        ...
        #pragma unroll
        for (int u=0; u < Unroll; u++) {
          got[u] = loadPack<Pack>(input + peers[u]*nStrideElts, packs[u]*EltPerPack, nElts);
        }
        #pragma unroll
        for (int u=0; u < Unroll; u++) {
          prim.sendLL(peers[u], rank*nIterPacks + packs[u], got[u]);
        }
      }
...
__device__ __forceinline__ void ncclSymRun_ReduceScatter_LL(ncclSymDevArgs const* args) {
  ...
  if (__builtin_expect(lowBits%8 == 0, true)) {
    // NOTE: this specialization appears to be of negligible help on sm90
