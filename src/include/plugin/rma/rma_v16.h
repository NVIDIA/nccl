...
typedef ncclRmaConfig_v15_t ncclRmaConfig_v16_t;

typedef struct {
  // True when the backend flushes all previously-received puts on any received signal, from any peer.
  // Reported per collComm: the AND over all NICs the collComm uses.
  bool flushesAllPutsOnAnySignal;
} ncclRmaProperties_v16_t;

// Same as the prior version, plus a per-collComm getRmaProperties query.
typedef struct {
  const char* name;
  ncclResult_t (*init)(void** ctx, uint64_t commId, ncclDebugLogger_t logFunction);
  ncclResult_t (*devices)(int* ndev);
  // Get the RMA properties for `collComm`.
  ...
  ncclResult_t (*createContext)(void* collComm, ncclRmaConfig_v16_t* config, void** rmaCtx);
