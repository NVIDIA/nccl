...
#ifdef _WIN32
// IB-specific params are not available on Windows, stub them
static inline int64_t ncclParamIbDataDirect() { return 0; }
static inline int64_t ncclParamDmaBufEnable() { return 0; }
#else
                                          ...
                                          void **gdrHandle, bool forceHost = false)
{
  ...
  else
  {
    ...
    *devPtr = *ptr;
    if (gdrHandle)
      *gdrHandle = NULL; // Mark as host allocated by nulling GDR handle
  }
  return ncclSuccess;
}
...
#ifdef _WIN32
  // Windows doesn't support DMA-BUF (it's a Linux kernel feature)
  (void)addr;
  (void)length;
  (void)fd;
  (void)forceNonDataDirect;
  return ncclInvalidUsage;
#else
...
ncclResult_t ncclGinProxyDestroyContext(ncclGin_t *ginComm, void *ginCtx)
{
  ...
  if (ctx)
  {
    ...
    {
      if (devHandle->handle)
        ncclCudaFree((void *)devHandle->handle);
      free(devHandle);
    }
...
ncclResult_t ncclGinProxyProgress(ncclGin_t *ginComm, void *ginCtx)
{
  ...
  for (int targetRank = 0; targetRank < ctx->comm->nRanks; targetRank++)
  {
    ...
    if (ginComm->ginProgress)
      ginComm->ginProgress(ctx->collComm);
  }
