...
static ncclResult_t sendProxySetup(struct ncclProxyConnection *connection, struct ncclProxyState *proxyState, void *reqBuff, int reqSize, void *respBuff, int respSize, int *done)
{
  ...
  if ((resources->maxCollBytes <= 0) || (resources->maxCollBytes > NCCL_MAX_NET_SIZE_BYTES))
  {
    WARN("sendProxySetup: collnet plugin returned invalid value for maxCollBytes %zu "
         "[allowed range: 0 - %llu]",
         resources->maxCollBytes, (unsigned long long)NCCL_MAX_NET_SIZE_BYTES);
    return ncclInternalError;
  }
  return ncclSuccess;
}
...
static ncclResult_t recvProxySetup(struct ncclProxyConnection *connection, struct ncclProxyState *proxyState, void *reqBuff, int reqSize, void *respBuff, int respSize, int *done)
{
  ...
  if (respSize != sizeof(collNetHandle_t))
    return ncclInternalError;
...
static ncclResult_t sendProxyFree(struct ncclProxyConnection *connection, struct ncclProxyState *proxyState)
{
  ...
  if (resources)
  {
    ...
    if (ncclAtomicRefCountDecrement(&connection->collNet->refCount) == 0)
      free(connection->collNet);
  ...
  if (args->state == ncclProxyOpReady)
  {
    for (int s = 0; s < args->nsubs; s++)
    {
      ...
      if (sub->reg && sub->isOneRPN)
        sub->nsteps = DIVUP((size_t)sub->nbytes, resources->maxCollBytes);
    }
    ...
    for (int s = 0; s < args->nsubs; s++)
    {
      ...
      if (ordered && (sub->transmitted < sub->received))
      {
        if (LAST_OF_GROUP(args, s))
        {
          ...
          else
          {
            ...
            if (nBytes > 0 && sub->requests[buffSlot] == nullptr)
              continue;
          }
        }
      ...
      if (LAST_OF_GROUP(args, s) && sub->done < sub->transmitted)
      {
        ...
        if (done)
        {
          ...
          for (int i = groupStart; i <= s; i++)
            args->subs[i].done += args->sliceSteps;
          args->idle = 0;
          int allDone = 1;
  ...
  if (args->state == ncclProxyOpProgress)
  {
    ...
    for (int s = 0; s < args->nsubs; s++)
    {
      ...
      if (LAST_OF_GROUP(args, s) && (sub->flushed < sub->received))
      {
        ...
        int done = 1;
        if (sub->requests[buffSlot])
          NCCLCHECK(proxyState->ncclCollNet->test(sub->requests[buffSlot], &done, NULL));
        if (done)
        {
          sub->requests[buffSlot] = nullptr;
          TRACE(NCCL_NET, "recvProxy [%ld/%d/%d] flushed", (long)sub->flushed, group, buffSlot);
          for (int i = group * COLLNET_GROUP_NSUBS; i <= s; i++)
            args->subs[i].flushed += args->sliceSteps;
          args->idle = 0;
          // continue;
        }
      }
...
ncclResult_t ncclCollNetSetup(ncclComm_t comm, ncclComm_t parent, struct ncclTopoGraph *graphs[])
{
  ...
  if (!comm->nvlsSupport)
  {
    ...
    for (int c = 0; c < collNetGraph->nChannels; c++)
    {
      ...
      if (mask != mask0)
        comm->collNetHeads[comm->collNetHeadsNum++] = head;
    }
  }
  else
  {
  ...
  else
  {
    ...
    for (int c = 0; c < comm->nChannels; c++)
    {
      ...
      for (int h = 0; h < comm->collNetHeadsNum; h++)
      {
        ...
        if (!collNetSetupFail)
          collNetSetupFail |= ncclTransportCollNetSetup(comm, collNetGraph, channel, head, head, h, collNetSend, &connect);
      }
  ...
  else
  {
    do
    {
      ...
      for (int h = 0; h < comm->collNetHeadsNum; h++)
        isHead |= (comm->collNetHeads[h] == comm->rank);
      if (isHead)
      {
