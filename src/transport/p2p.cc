...
static ncclResult_t p2pSendConnect(struct ncclComm *comm, struct ncclConnect *connectInfo, int nranks, int rank, struct ncclConnector *send)
{
  ...
  for (int p = 0; p < NCCL_NUM_PROTOCOLS; p++)
  {
    if (info->read && p == NCCL_PROTO_SIMPLE)
    {
      ...
      if (resources->sendDevMem == NULL)
        return ncclInternalError; // We should not use read + memcpy
  ...
  if (regRecord)
  {
    ...
    for (int p = 0; p < nPeers; p++)
    {
      ...
      if (regRecord->ipcInfos[peerLocalRank])
      {
        ...
        *regBufFlag = 1;
        if (isLegacyIpc)
          *isLegacyIpc = regRecord->ipcInfos[peerLocalRank]->impInfo.legacyIpcCap;
...
fail:
  ...
  {
    if (mapped)
      CUCHECK(cuMemUnmap((CUdeviceptr)regAddr, ipcExpInfo->size));
    if (regAddr)
      CUCHECK(cuMemAddressFree((CUdeviceptr)regAddr, ipcExpInfo->size));
    if (imported)
