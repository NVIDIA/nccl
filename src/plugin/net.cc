...
// Internal plugins count: 2 on Linux (IB and Socket), 1 on Windows (Loopback)
#ifdef _WIN32
...
static ncclResult_t ncclNetPluginInit(struct ncclComm *comm, netPluginLib_t *pluginLib)
{
  ...
  if (pluginLib->ncclNetPluginState >= ncclNetPluginStateInitReady && pluginLib->ncclNet)
  {
    ...
    if (pluginLib->ncclNet->init(&comm->netContext, comm->commHash, &commConfig, ncclDebugLog, ncclProfilerCallback) != ncclSuccess)
      goto fail;
  }
  // Detection of the devices is only done when the plugin is being initialized the first time
  if (pluginLib->ncclNetPluginState == ncclNetPluginStateInitReady && pluginLib->ncclNet)
  {
    if (pluginLib->ncclNet->devices(&ndev) != ncclSuccess || ndev <= 0)
      goto fail;
  ...
  if (pluginLib->ncclGinPluginState == ncclNetPluginStateInitReady && pluginLib->ncclGin)
  {
    ...
    else if (pluginLib->ncclGin->devices(&ndev) != ncclSuccess || ndev <= 0)
      pluginLib->ncclGinPluginState = ncclNetPluginStateDisabled;
    else
    {
...
{
  if (ncclSuccess != ncclNetCheckDeviceVersion(comm, netPluginLibs[pluginIndex].ncclNet, 0))
    goto fail;
...
static void initPluginLibsOnceFunc()
{
...
#ifndef _WIN32
  // Add 2 internal ib and socket plugins (Linux only - require POSIX sockets and InfiniBand)
  ...
  netPluginLibs[pluginCounter++].ncclNetPluginState = ncclNetPluginStateInitReady;
#else
  // Windows: Add socket plugin for multi-GPU communication
  netPluginLibs[pluginCounter].ncclNet = &ncclNetSocket;
  netPluginLibs[pluginCounter].ncclGin = NULL;
  netPluginLibs[pluginCounter].ncclNetPluginState = ncclNetPluginStateInitReady;
  netPluginLibs[pluginCounter].ncclGinPluginState = ncclNetPluginStateLoadFailed;
  ++pluginCounter;
#endif
...
static ncclResult_t ncclNetPluginFinalize(struct ncclComm *comm, int pluginIndex)
{
  ...
  if (netPluginLibs[pluginIndex].ncclCollNet && netPluginLibs[pluginIndex].ncclCollNetPluginState == ncclNetPluginStateEnabled)
    NCCLCHECK(netPluginLibs[pluginIndex].ncclCollNet->finalize(comm->collNetContext));
  if (netPluginLibs[pluginIndex].ncclGin && netPluginLibs[pluginIndex].ncclGinPluginState == ncclNetPluginStateEnabled)
    NCCLCHECK(netPluginLibs[pluginIndex].ncclGin->finalize(comm->ginContext));
  netPluginLibs[pluginIndex].ncclNetPluginRefCount--;
  if (pluginIndex < (pluginCount - NCCL_NET_NUM_INTERNAL_PLUGINS))
  {
...
ncclResult_t ncclNetInit(struct ncclComm *comm)
{
  ...
  for (int pluginIndex = 0; pluginIndex < pluginCount; pluginIndex++)
  {
    ...
    if ((netPluginLibs[pluginIndex].ncclNetPluginState >= ncclNetPluginStateInitReady) && (!comm->config.netName || (strcasecmp(comm->config.netName, netPluginLibs[pluginIndex].ncclNet->name) == 0)))
    {
...
{
  if (netPluginLibs[netPluginIndex].ncclNetPluginState != ncclNetPluginStateEnabled || nVirtDevs < 0)
    goto fail;
  // lock not needed as it's called within a lock already in ncclTopoGetSystem
...
{
  if (netPluginLibs[netPluginIndex].ncclCollNetPluginState != ncclNetPluginStateEnabled || nVirtDevs < 0)
    goto fail;
  // lock not needed as it's called within a lock already in ncclTopoGetSystem
