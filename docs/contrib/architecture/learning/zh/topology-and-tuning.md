...
> 在日志里看到它们：下面每个阶段都可以用 `NCCL_DEBUG=INFO` 加上
> `NCCL_DEBUG_SUBSYS` 过滤来观察。拓扑、路径与图搜索在 `GRAPH` 下打印；
> transport 建连在 `INIT` 下打印（transport 细节可看 `NET`/`P2P`/`SHM`）；
> 调优模型在 `TUNING` 下打印。
...
想看探测出的机器图，可用 `NCCL_DEBUG=INFO NCCL_DEBUG_SUBSYS=GRAPH`。
...
比较、可排序”的路径类别。这些路径类别同样会在 `NCCL_DEBUG_SUBSYS=GRAPH` 下打印。
...
擎”。搜出的图会在 `NCCL_DEBUG_SUBSYS=GRAPH` 下打印。
...
这一步生成的逐 rank channel 布局可在 `NCCL_DEBUG_SUBSYS=INIT` 下看到。
...
`src/transport.cc` 里的顺序很关键，因为 `selectTransport(...)` 会遍历 transport
列表，停在第一个 `canConnect` 回答“能”的项上。有两个细节值得澄清：

- 它只遍历 `NTRANSPORTS` 个真正的 transport（P2P、SHM、NET、CollNet）。profiler
  transport（`TRANSPORT_PROFILER`）排在 `NTRANSPORTS` 之外，这里从不会问它。
- CollNet 的 `canConnect` 对一条普通点对点（p2p）边总是回答“不能”（CollNet 是
  通过另一条专门的 collective 路径建立的）。所以逐边能胜出的实际上是 P2P、SHM
  或 NET。

可用 `NCCL_DEBUG=INFO NCCL_DEBUG_SUBSYS=INIT` 观察（transport 细节可补上
`NET`/`P2P`/`SHM`）。
...
接拿来做选择。这些生成的表会在 `NCCL_DEBUG_SUBSYS=TUNING` 下打印。
