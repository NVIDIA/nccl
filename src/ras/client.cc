...
static void parseArgs(int argc, char **argv)
{
  ...
  while ((c = getopt_long(argc, argv, "f:h:p:t:v", longOpts, &optIdx)) != -1)
  {
    ...
    case 'e':
      ...
      exit(0);
    case 'r':
      fprintf(stderr, "NCCL RAS client version " STR(NCCL_MAJOR) "." STR(NCCL_MINOR) "." STR(NCCL_PATCH) NCCL_SUFFIX "\n");
      exit(0);
    default:
      printUsage(argv[0]);
      exit(1);
...
retry:
  ...
  if (sock == -1)
  {
    fprintf(stderr, "Failed to connect to the NCCL RAS service!\n"
                    ...
                    "%s.\n",
            (strcmp(hostName, "localhost") || strcmp(port, STR(NCCL_RAS_CLIENT_PORT)) ? "the host/port arguments are correct and match NCCL_RAS_ADDR" : "the RAS client was started on a node where the NCCL job is running"));
    goto fail;
  }
