...
class Primitives<T, RedOp, Fan, Direct, ProtoLL, P2p, isNetOffload> : public PrimitivesWithoutDirect<Primitives<T, RedOp, Fan, Direct, ProtoLL, P2p, isNetOffload>>
{
  ...
  // Same for MaxSend - MSVC doesn't allow zero-sized arrays
  ...
  inline __device__ void waitSend(int nbytes)
  {
    if (sendConnHeadPtr)
    {
      ...
      if (sendConnFifo)
      {
        ...
        // Use system-scope store for GPU→CPU visibility on Windows without P2P
        // C-style cast needed to remove volatile qualifier since st.sys provides stronger ordering
        st_relaxed_sys_global((int64_t *)&sendConnFifo[sendConnHead % NCCL_STEPS].size, (int64_t)size);
      }
      sendConnHead += 1;
    }
    barrier();
...
public:
  __device__ Primitives(
      ...
      bool ipcReg = false, bool netReg = false, int stepSize_ = 0) : redOp(redOpArg),
