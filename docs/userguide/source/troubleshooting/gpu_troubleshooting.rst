...
**False peer-access grants and private mirrors.** Capability checks such as ``cudaDeviceCanAccessPeer``, ``nvidia-smi topo -p2p``, and round-trip / bandwidth microbenchmarks can all report success while peer stores never become visible to the owning GPU. In that failure class, a writer GPU reads its own pattern back through the peer window (a private per-direction mirror), but the owner still sees zeros. NCCL then hangs polling peer flags, or — with :ref:`env_NCCL_P2P_USE_CUDA_MEMCPY` enabled — can complete collectives with silently wrong results. Owner-local verification after a one-way copy (as ``simpleP2P`` does) is required; do not trust write-then-readback through the same peer window. If you hit this, set :ref:`env_NCCL_P2P_DISABLE`\ ``=1`` or restrict :ref:`env_NCCL_P2P_LEVEL` (for example to ``PXB``). Optionally set :ref:`env_NCCL_P2P_VALIDATE`\ ``=1`` so NCCL disables P2P for pairs that fail a device peer-store + owner-local probe during initialization (and fail closed when a peer is invisible so the probe cannot run). See also `NCCL issue #2335 <https://github.com/NVIDIA/nccl/issues/2335>`__.
