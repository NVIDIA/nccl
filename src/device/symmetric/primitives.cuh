...
struct ncclSymPrims {
  ...
  __device__ ncclSymPrims(ncclSymDevComm const &comm):
    rank(comm.rank),
    nRanks(comm.nRanks),
    ...
    base(comm.base),
    offsetMc((char*)comm.baseMc - (char*)base),
    stride4G(comm.stride4G),
    nvlAtomics(false && /*ignore*/comm.nvlAtomics) {

    barEpoch = base->barEpochs[block];
    llEpoch = base->llEpochs[block] + 2;
  }
  __device__  ~ncclSymPrims() {
    if (threadIdx.x == 0) {
      base->barEpochs[block] = barEpoch;
      base->llEpochs[block] = llEpoch - 2;
    }
  }
  ...
  __device__  void barrierArrive(ncclCoopCta cta, bool release) {
    cta.sync();
    int r = threadIdx.x;
    int delta = nvlAtomics ? nRanks-1 : 1;
    if (r != rank && r < nRanks) {
      if (nvlAtomics) {
        uint32_t* inbox = &peerPtr(r, base)->barInboxPerPeer[block*nRanks + r];
        if (release) {
          asm volatile("red.release.sys.add.u32 [%0],1;" :: "l"(inbox));
        } else {
          asm volatile("red.relaxed.sys.add.u32 [%0],1;" :: "l"(inbox));
        }
      } else {
        uint32_t* inbox = &peerPtr(r, base)->barInboxPerPeer[block*nRanks + rank];
        if (release) {
          asm volatile("st.release.sys.u32 [%0],%1;" :: "l"(inbox), "r"(barEpoch+delta));
        } else {
          asm volatile("st.relaxed.sys.u32 [%0],%1;" :: "l"(inbox), "r"(barEpoch+delta));
        }
      }
    }
  }
  ...
  __device__  void barrierWait(ncclCoopCta cta, bool acquire) {
    int r = threadIdx.x;
    int delta = nvlAtomics ? nRanks-1 : 1;
    if (nvlAtomics ? (r == rank) : (r != rank && r < nRanks)) {
      ...
      while (true) {
        ...
        if (got-(barEpoch+delta) <= uint32_t(-1)>>1) break;
      }
    }
    barEpoch += delta;
    cta.sync();
