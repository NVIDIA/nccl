...
> 社区贡献，非 NCCL 团队官方资料。作者/译者：tianhao909
> (<tianhaofu@foxmail.com>)。基于 NCCL v2.30u1 (2.30.6)，按 Apache-2.0 许可发布。
...
flowchart TB
    App["框架 / 用户应用"]
    API["公共 API<br/>src/init.cc<br/>src/collectives.cc"]
    Comm["ncclComm 状态<br/>channels / graphs / cost tables"]
    Boot["src/bootstrap.cc<br/>带外握手与交换"]
    Topo["src/graph/topo.cc<br/>paths.cc<br/>search.cc<br/>connect.cc"]
    Tune["src/graph/tuning.cc<br/>+ tuner 插件"]
    Plan["src/enqueue.cc<br/>planner 与 launch 准备"]
    Proxy["src/proxy.cc"]
...
### 用 NCCL_DEBUG_SUBSYS 观察初始化

上面这些决策大多可以被“看见”。先设 `NCCL_DEBUG=INFO`，再用
`NCCL_DEBUG_SUBSYS` 缩小输出范围：

| 初始化环节 | 推荐子系统 | 会打印什么 |
| --- | --- | --- |
| 带外握手 | `BOOTSTRAP` | bootstrap ring 建立与 rank 交换 |
| 拓扑 / 路径 / 图搜索 | `GRAPH` | 探测到的硬件图、path type、以及搜出的 ring/tree/NVLS/CollNet 图 |
| channel 与 transport 建连 | `INIT` | 初始化总体进度与逐 peer 连接建立 |
| transport 细节 | `NET`、`P2P`、`SHM` | 某条边最终由哪种具体 transport 承担 |
| 性能模型 | `TUNING` | 为各 algo/protocol 生成的延迟/带宽表 |
| NVLS / 对称内存 | `NVLS` | NVLS 相关的建立 |

例如，`NCCL_DEBUG=INFO NCCL_DEBUG_SUBSYS=GRAPH` 是看清“为什么选了某个 ring/tree”
最快的方式；而 `NCCL_DEBUG_SUBSYS=INIT` 则能看到每条 peer 边是怎么连上的。
...
在 `src/transport.cc` 中，NCCL 维护了一张 transport 候选表。数组 `ncclTransports[]`
按顺序包含：
...
还有一个 profiler transport（`TRANSPORT_PROFILER`），但它排在 `NTRANSPORTS` 计数
*之外*，所以 `selectTransport(...)` 从不会问它。选择逻辑只会遍历上述
`NTRANSPORTS` 个项，逐个问“你能接这个 peer pair 吗？”谁先回答“能”谁就赢。

有一点值得澄清：CollNet 的 `canConnect` 对一条普通点对点（p2p）边总是回答
“不能”（CollNet 是通过另一条专门的 collective 路径建立，而不是走这里的逐边
选择）。所以实际上能在这里拿下一条 peer 边的，是 P2P、SHM、NET。
...
flowchart LR
    ...
    Select --> CN["collNetTransport（canConnect 对 p2p 恒为 no）"]
```

可用 `NCCL_DEBUG=INFO NCCL_DEBUG_SUBSYS=INIT` 观察这个选择过程（transport 细节
可补上 `NET`/`P2P`/`SHM`）。
