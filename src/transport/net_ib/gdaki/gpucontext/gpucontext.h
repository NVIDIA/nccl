...
constexpr size_t NCCL_GIN_GDAKI_GPU_CONTEXT_MAX_SIZE = std::max({sizeof(ncclGinGdakiGPUContext_v1),
                                                                 sizeof(ncclGinGdakiGPUContext)});

const int NCCL_GIN_GDAKI_GPU_CONTEXT_VERSION = 2;

void ncclGinGdakiGPUContext_initCurrent(void* ctxArray, int idx,
...
ncclResult_t ncclGinGdakiGPUContext_init(int version, void* ctxArray, int idx,
                                         struct doca_gpu_dev_verbs_qp* gdqp,
                                         struct doca_gpu_dev_verbs_qp* companion_gdqp,
                                         struct ncclGinGdakiGlobalGPUBufferTable<uint64_t> counters_table,
                                         struct ncclGinGdakiGlobalGPUBufferTable<uint64_t> signals_table,
  ...
  switch (version) {
    case 1:
      ncclGinGdakiGPUContext_v1_init(
          ctxArray, idx, gdqp, companion_gdqp, counters_table, signals_table, sink_buffer_lkey);
      break;
    case NCCL_GIN_GDAKI_GPU_CONTEXT_VERSION:
      ncclGinGdakiGPUContext_initCurrent(
          ctxArray, idx, gdqp, companion_gdqp, counters_table, signals_table, sink_buffer_lkey,
          last_issued_get, last_visible_get);
      break;
    default:
