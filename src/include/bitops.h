...
/* Platform-specific intrinsics for MSVC
 * These are needed both for pure MSVC compilation and for nvcc with MSVC host compiler.
 * When nvcc compiles device code (__CUDA_ARCH__ is defined), it uses nvcc's intrinsics.
 * When compiling host code with MSVC (even under nvcc), we need these MSVC equivalents.
 */
#if defined(_MSC_VER)
#include <intrin.h>

/* Only define these for host compilation paths */
#if !defined(__CUDA_ARCH__)

/* Count leading zeros - MSVC doesn't have __builtin_clz */
static __forceinline int __nccl_builtin_clz(unsigned int x)
{
  unsigned long index;
...
#define __builtin_clz __nccl_builtin_clz
#endif

/* __builtin_clzl for unsigned long (32-bit on Windows) */
static __forceinline int __nccl_builtin_clzl(unsigned long x)
{
  unsigned long index;
  if (_BitScanReverse(&index, x))
  {
...
#define __builtin_clzl __nccl_builtin_clzl
#endif

static __forceinline int __nccl_builtin_clzll(unsigned long long x)
{
  unsigned long index;
#if defined(_WIN64)
...
#define __builtin_clzll __nccl_builtin_clzll
#endif

/* Find first set (lowest set bit) - returns 1-indexed position, 0 if none */
static __forceinline int __nccl_builtin_ffs(int x)
{
  unsigned long index;
...
#define __builtin_ffs __nccl_builtin_ffs
#endif

static __forceinline int __nccl_builtin_ffsl(long x)
{
  unsigned long index;
  if (_BitScanForward(&index, (unsigned long)x))
    return (int)(index + 1);
...
#define __builtin_ffsl __nccl_builtin_ffsl
#endif

static __forceinline int __nccl_builtin_ffsll(long long x)
{
  unsigned long index;
#if defined(_WIN64)
  if (_BitScanForward64(&index, (unsigned long long)x))
    return (int)(index + 1);
#else
  if (_BitScanForward(&index, (unsigned long)x))
    return (int)(index + 1);
...
#define __builtin_ffsll __nccl_builtin_ffsll
#endif

/* Population count (count set bits) */
static __forceinline int __nccl_builtin_popcount(unsigned int x)
{
  return (int)__popcnt(x);
}
#ifndef __builtin_popcount
#define __builtin_popcount __nccl_builtin_popcount
#endif

static __forceinline int __nccl_builtin_popcountl(unsigned long x)
{
  return (int)__popcnt(x);
}
#ifndef __builtin_popcountl
#define __builtin_popcountl __nccl_builtin_popcountl
#endif

static __forceinline int __nccl_builtin_popcountll(unsigned long long x)
{
  return (int)__popcnt64(x);
}
#ifndef __builtin_popcountll
#define __builtin_popcountll __nccl_builtin_popcountll
#endif

/* Byte swap */
static __forceinline unsigned short __nccl_builtin_bswap16(unsigned short x)
{
  return _byteswap_ushort(x);
}
#ifndef __builtin_bswap16
#define __builtin_bswap16 __nccl_builtin_bswap16
#endif

static __forceinline unsigned int __nccl_builtin_bswap32(unsigned int x)
{
  return _byteswap_ulong(x);
}
#ifndef __builtin_bswap32
#define __builtin_bswap32 __nccl_builtin_bswap32
#endif

static __forceinline unsigned long long __nccl_builtin_bswap64(unsigned long long x)
{
  return _byteswap_uint64(x);
}
#ifndef __builtin_bswap64
#define __builtin_bswap64 __nccl_builtin_bswap64
#endif

/* Branch prediction hint - no-op on MSVC */
