...
{
  // MSVC doesn't allow zero-sized arrays, so ensure minimum size of 1
  ...
  __device__ __forceinline__ void waitPeer(intptr_t srcIx, intptr_t dstIx, int offset, int nelts)
  {
    ...
    if (flags & (Recv * RoleWaitRecv | Send * RoleWaitSend))
    {
      if ((flags & ConnFifoEnabled) && (flags & (Send * RoleWaitSend)))
        st_relaxed_sys_global(reinterpret_cast<int64_t *>(&connFifo[step % NCCL_STEPS].size), (int64_t)(nelts * sizeof(T)));
...
#pragma unroll 1
      ...
      do
      {
        ...
        if (tid == 0)
        {
          ...
          if (Dst)
            ncclShmem.groups[group].dsts[0] = (DstBuf == Input ? userInput : userOutput) + dstIx + offset;
        }
            ...
            && MultimemSrcs == 0 && MultimemDsts == 0 && !Src)
        {
          ...
          {
            reduceCopy<Unroll, RedOp, T, 0, 1, 1, 0, 1, MaxSend, /*PreOpSrcs*/ 0>(tid, nworkers, /*redArg*/ 0, /*preOpArgs*/ nullptr, /*postOp*/ false,
        ...
        else if (DirectSend && !DirectRecv && SrcBuf != Input && ncclShmem.groups[group].dsts[Dst] == nullptr)
        {
          ...
          reduceCopy<Unroll, RedOp, T, 0, 1, 1, 0, 1, 1, /*PreOpSrcs*/ 0>(tid, nworkers, ncclShmem.redOpArgs[0], nullptr, postOp,
        ...
        {
          constexpr int PreOpSrcs = SrcBuf != Input ? 0 : DirectRecv * MaxRecv == NCCL_MAX_DIRECT_ARITY ? (1 + NCCL_MAX_DIRECT_ARITY)
                                                                                                        : 1;
          if (Send && Dst && ncclShmem.groups[group].dsts[1] == nullptr)
          {
            ...
            reduceCopy<Unroll, RedOp, T,
                       ...
                       0, 1, 1, PreOpSrcs>(tid, nworkers, ncclShmem.redOpArgs[0], ncclShmem.redOpArgs, postOp,
          ...
          else
          {
            reduceCopy<Unroll, RedOp, T,
                       ...
                       MultimemDsts, Send + Dst, Send * MaxSend + Dst, PreOpSrcs>(tid, nworkers, ncclShmem.redOpArgs[0], ncclShmem.redOpArgs, postOp,
                                                                                  Recv * fan.nrecv() + Src, ncclShmem.groups[group].srcs,
...
#pragma unroll 1
    for (int slice = 0; slice < SlicePerChunk; slice++)
    {
      if (tid < nworkers)
      {
        ...
        fn.template operator()<SlicePerChunk, 0, Recv * MaxRecv, 0, Send * MaxSend, MultimemSrcs, MultimemDsts>(tid, nworkers, slice, stepSize * StepPerSlice,
      ...
      if (flags & Send * RolePostSend)
      {
        ...
        if (flags & ConnFifoEnabled)
          st_relaxed_sys_global(reinterpret_cast<int64_t *>(&connFifo[step % NCCL_STEPS].size), (int64_t)(dstSize * sizeof(T)));
      }
      barrier();
...
#pragma unroll
        ...
        {
          if (tid == 0)
            ncclShmem.groups[group].dsts[0] = (T *)ncclShmem.groups[group].userOutput + outIx + offset;
    ...
    if (flags & RoleWaitRecv)
    {
      ...
      if (Direct)
      {
        ...
        if (netRegFlag)
        {
          ...
          {
            flags |= NetRegMode;
            st_relaxed_sys_global(reinterpret_cast<int64_t *>(&connFifo[step % NCCL_STEPS].size), (int64_t)0);
          }
        }
      }
    }
...
public:
  __device__ Primitives(
      ...
      struct ncclDevWorkP2p *p2pWork = nullptr, int stepSize_ = 0, int mode = primsModeDefault) : tid(tid), nthreads(nthreads), tidInBlock(threadIdx.x), group(group),
                                                                                                  stepSize(stepSize_ == 0 ? ncclShmem.comm.buffSizes[NCCL_PROTO_SIMPLE] / NCCL_STEPS / sizeof(T) : stepSize_)
  {
    ...
    { // Connect to ranks in sendPeers/recvPeers
      ...
      if (flags & (RoleWaitRecv | RolePostRecv))
        loadRecvConn(ncclShmem.channel.peers[peer], connIndexRecv, collWork ? collWork->direct : 0, recvIpcReg, recvNetReg);
      // coverity[overrun-call] => Coverity think prims.index can be greater than 1
      if (flags & (RoleWaitSend | RolePostSend))
        loadSendConn(ncclShmem.channel.peers[peer], connIndexSend, collWork ? collWork->direct : 0, sendIpcReg, sendNetReg);
  ...
  __device__ void setDataPtrs(void const *inputBuf, void *outputBuf, uint64_t redOpArg, struct ncclDevWorkCollReg *work, uint8_t ipcReg, int peer)
  {
    ...
    if (Direct && ipcReg)
    {
      ...
      if (sendAcceptor)
      {
        ...
        {
          ptr = *slot;
          if (ptr != nullptr || checkAbort(flags, Aborted, spins))
            break;
        }
      ...
      if (recvAcceptor)
      {
        ...
        if (slot && argSlot0 && argSlot1)
        {
          directBuff = reinterpret_cast<T *>(ptr);
          if (MaxSend != 0)
          { // reduce group rather than gather group
  ...
  __device__ __forceinline__ void patReduce(struct ncclPatStep *ps, struct ncclPatShmem *shmem)
  {
    ...
    reduceCopy<Unroll, RedOp, T, 0, 1, 2, 0, 1, 1, /*PreOpSrcs*/ 0>(tid, nthreads, ncclShmem.redOpArgs[0], nullptr, /*postOp=*/false,
    ...
    if (postSend && (flags & RolePostSend))
    {
      if (peer->connFifo)
      {
        st_relaxed_sys_global(reinterpret_cast<int64_t *>(&peer->connFifo[step % NCCL_STEPS].size), (int64_t)((ps->sendOffset + nelem) * sizeof(T)));
      }
  ...
  __device__ __forceinline__ void patCopy(struct ncclPatStep *ps, struct ncclPatShmem *shmem)
  {
    ...
    reduceCopy<Unroll, RedOp, T, 0, 1, 1, 0, 1, 2, /*PreOpSrcs*/ 0>(tid, nthreads, ncclShmem.redOpArgs[0], nullptr, /*postOp=*/false,
