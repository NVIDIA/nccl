...
ncclResult_t ncclEnqueueCheck(struct ncclInfo* info) {
  ...
  TRACE_CALL("nccl%s(%" PRIx64 ",%" PRIx64 ",%zu,%d,%d,%d,%p,%p) nNodes %d nRanks %d", info->opName, reinterpret_cast<int64_t>(info->sendbuff), reinterpret_cast<int64_t>(info->recvbuff), info->count, info->datatype, info->op, info->root, info->comm, info->stream, info->comm->nNodes, info->comm->nRanks);
