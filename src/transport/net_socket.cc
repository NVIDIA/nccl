...
static ncclResult_t ncclNetSocketGetPciPath(char *devName, char **pciPath)
{
  ...
  *pciPath = realpath(devicePath, NULL);
  ...
  // Windows doesn't have /sys filesystem - use NULL path
  *pciPath = NULL;
#endif
  return ncclSuccess;
}
...
#elif NCCL_PLATFORM_WINDOWS
  // Use Windows API to get interface speed
  if (ncclGetInterfaceSpeed(devName, speed) != 0 || *speed <= 0)
  {
    INFO(NCCL_NET, "Could not get speed for %s. Defaulting to 10 Gbps.", devName);
    *speed = 10000;
  }
  else
  {
    INFO(NCCL_NET, "NET/Socket: Interface %s speed: %d Mbps", devName, *speed);
  }
#endif
  return ret;
}
...
#elif NCCL_PLATFORM_WINDOWS
    // Windows doesn't have sysfs - use defaults
    TRACE(NCCL_NET, "Vendor auto-detection not available on Windows, using defaults");
#endif
    if (nThreads == -2)
      nThreads = autoNt;
...
ncclResult_t ncclNetSocketConnect(void *ctx, int dev, void *opaqueHandle, void **sendComm, ncclNetDeviceHandle_t ** /*sendDevComm*/)
{
  ...
  *sendComm = NULL;
  ...
  if (stage->state == ncclNetSocketCommStateConnect)
    goto socket_connect_check;
  if (stage->state == ncclNetSocketCommStateSend)
    goto socket_send;
...
ncclResult_t ncclNetSocketAccept(void *listenComm, void **recvComm, ncclNetDeviceHandle_t ** /*recvDevComm*/)
{
  ...
  *recvComm = NULL;
  if (stage->state == ncclNetSocketCommStateAccept)
    goto socket_accept_check;
  if (stage->state == ncclNetSocketCommStateRecv)
    goto socket_recv;
...
ncclResult_t ncclNetSocketTest(void *request, int *done, int *size)
{
  *done = 0;
  ...
  if (r->used == 1)
  { /* try to send/recv size (+ inline data if any) */
    int msgSize;
    ...
    if (r->op == NCCL_SOCKET_SEND)
    {
      ...
      if (inlineSize > 0)
        memcpy(msg + SOCKET_CTRL_SIZE, r->data, inlineSize);
    }
    else
    {
      ...
      if (receivedInline > 0)
        memcpy(r->data, msg + SOCKET_CTRL_SIZE, receivedInline);
...
ncclResult_t ncclNetSocketClose(void *opaqueComm)
{
  ...
  if (comm)
  {
    ...
    for (int i = 0; i < comm->nSocks; i++)
    {
      ...
      if (ready)
        NCCLCHECK(ncclSocketClose(&comm->socks[i]));
    }
    if (comm->inlineData)
      free(comm->inlineData);
    delete comm;
  }
  return ncclSuccess;
}
