...
#include "device.h"
#include "platform.h" /* For ssize_t and atomic operations on Windows */

#define NCCL_MAX_NET_SIZE (1024LL * 1024 * 1024) // Rather than send INT_MAX which is 2G-1, send a power of two.
...
public:
  __device__ __host__ PatRSAlgorithm(int stepSize, int stepDepth, int maxParallelFactor, size_t offset, size_t end, size_t count, int chunkCount, int rank, int nranks) : offset(offset), end(end), count(count), chunkCount(chunkCount), rank(rank), nranks(nranks)
  {
    ...
    postFreq = aggFactor;
    if (postFreq < parallelFactor)
      parallelFactor = postFreq;
    int d = stepDepth;
  ...
  __device__ __host__ void getNextOp(struct ncclPatStep *ps)
  {
    ...
    else if (phase == 1)
    {
      ...
      ps->postSend = 0;
      if (ps->recvDim == 0 && (((a % postFreq) + 1 >= postFreq) || (a == lastA - 1)))
        ps->postSend = 1;
      if (((a % postFreq) + 1 >= postFreq) || (a == lastA - 1))
      {
        ps->postRecv = 1;
      ...
      if (recvDim > 0 && (((a - sendSkipped) % postFreq) + 1 >= postFreq) && skip == 0)
        stepOffset++;
    }
    else if (phase == 2)
    {
    ...
    else if (phase == 3)
    {
      ...
      if ((foffset % postFreq) + 1 >= postFreq && skip == 0)
        stepOffset++;
    ...
    if (a >= lastA && a >= parallelFactor)
    {
      ...
      if (p == 3)
        scale *= 2;
      phase =
          p == 0 ? as == 1 ? (aggFactor > 1 ? 2 : 4) : 1 : p == 1 ? as % 2 == 1 ? 0 : 1
                                                       : p == 2   ? 3
...
public:
  __device__ __host__ PatAGAlgorithm(int stepSize, int stepDepth, int maxParallelFactor, size_t offset, size_t end, size_t count, int chunkCount, int rank, int nranks) : offset(offset), end(end), count(count), chunkCount(chunkCount), rank(rank), nranks(nranks)
  {
    parallelFactor = maxParallelFactor;
    aggDelta = nrPow2 = (1 << log2Up(nranks));
