...
// Weak declaration to detect optional libibverbs support of recent feature.
// Directly use ASSIGN_SYM instead once the feature becomes standard and should always exist.
struct ibv_mr*  __attribute__((weak)) ibv_reg_dmabuf_mr(struct ibv_pd *pd, uint64_t offset,
         size_t length, uint64_t iova, int fd, int access);

struct ibv_mr *ibv_internal_reg_dmabuf_mr(struct ibv_pd *pd, uint64_t offset,
				 size_t length, uint64_t iova, int fd,
				 int access) {
  if (ibv_reg_dmabuf_mr) {
...
ncclResult_t buildIbvSymbols(struct ncclIbvSymbols* ibvSymbols) {
  ...
  ibvSymbols->ibv_internal_reg_dmabuf_mr = &ibv_internal_reg_dmabuf_mr;
