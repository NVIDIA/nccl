...
#define DEFAULT_LL_BUFFSIZE (NCCL_LL_LINES_PER_THREAD*NCCL_LL_MAX_NTHREADS*NCCL_STEPS_DEFAULT*sizeof(union ncclLLFifoLine))
#define DEFAULT_LL128_BUFFSIZE (NCCL_LL128_ELEMS_PER_THREAD*NCCL_LL128_MAX_NTHREADS*NCCL_STEPS_DEFAULT*sizeof(uint64_t))
...
static ncclResult_t computeBuffSizes(struct ncclComm* comm) {
  ...
  // Scale up buffer sizes for deeper buffer (NCCL_STEPS)
  int size_mult = 1;
  for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
    if (comm->interLat[a] > 100) {
      size_mult = NCCL_MAX_STEPS/NCCL_STEPS_DEFAULT;
      break;
    }
  }

  for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    comm->buffSizes[p] = comm->hostDevComm.buffSizes[p] = envs[p] != -2 ? envs[p] : defaults[p] * size_mult;
  }
  return ncclSuccess;
}
