...
#else
// Windows: GDR not supported, but provide stub macros
#define GDRLOCK() ((void)0)
#define GDRUNLOCK() ((void)0)
#define GDRLOCKCALL(cmd, ret) \
  do                          \
  {                           \
    ret = cmd;                \
...
#elif defined(__x86_64__) || (defined(_MSC_VER) && defined(_M_X64))
#include <immintrin.h>
static inline void wc_store_fence(void) { _mm_sfence(); }
#elif defined(__aarch64__) || (defined(_MSC_VER) && defined(_M_ARM64))
#ifdef __cplusplus
#include <atomic>
...
static gdr_t wrap_gdr_open(void)
{
  gdr_t g = gdr_open();
  return g;
}
static ncclResult_t wrap_gdr_close(gdr_t g)
{
  GDRCHECK(gdr_close(g));
  return ncclSuccess;
}
