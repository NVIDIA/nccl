...
uint64_t getPidHash(void)
{
  ...
  if (len < 0)
    len = 0;
#else
  // On Windows, there's no namespace concept like /proc/self/ns/pid
  // Use GetCurrentProcessId() which is already done above via getpid()
  int len = 0;
#endif
...
NCCL_THREAD_LOCAL struct ncclThreadSignal ncclThreadSignalLocalInstance = ncclThreadSignalStaticInitializer();
