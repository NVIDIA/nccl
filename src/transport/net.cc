...
#define NCCL_NET_MAP_GET_POINTER(mapStruct, cpuOrGpu, offsetName) \
  (NCCL_NET_MAP_OFFSET_NULL(mapStruct, offsetName) ? NULL : (mapStruct)->mems[NCCL_NET_MAP_OFFSET_BANK(mapStruct, offsetName)].cpuOrGpu##Ptr + ((mapStruct)->offsets.offsetName & NCCL_NET_MAP_MASK_OFFSET))
...
static void populateCommNetAttrs(struct ncclComm *comm, struct ncclConnector *conn, ncclNetAttr_t *netAttr)
{
  *netAttr = NCCL_NET_ATTR_INIT;
  ...
  if (conn->p2pOnly)
  {
    ...
    if (comm->nRanks < maxConcPeers)
      maxConcPeers = comm->nRanks;
...
static ncclResult_t sendSetup(struct ncclComm *comm, struct ncclTopoGraph *graph, struct ncclPeerInfo *myInfo, struct ncclPeerInfo *peerInfo, struct ncclConnect *connectInfo, struct ncclConnector *send, int channelId, int connIndex)
{
  ...
  send->conn.shared = req.shared = graph || connIndex == 0 ? 0 : ncclParamNetSharedBuffers() != -2 ? ncclParamNetSharedBuffers()
                                                                                                   : 1;
...
static ncclResult_t recvSetup(struct ncclComm *comm, struct ncclTopoGraph *graph, struct ncclPeerInfo *myInfo, struct ncclPeerInfo *peerInfo, struct ncclConnect *connectInfo, struct ncclConnector *recv, int channelId, int connIndex)
{
  ...
  recv->conn.shared = req.shared = graph || connIndex == 0 ? 0 : ncclParamNetSharedBuffers() != -2 ? ncclParamNetSharedBuffers()
                                                                                                   : 1;
  req.channelId = channelId;
  req.connIndex = connIndex;
                                         ...
                                         int nChannels, char **gpuPtr, char **cpuPtr, int *size, ncclIpcDesc *ipcDesc)
{
  ...
  if (!cuda && state->hostBuff == NULL)
  {
    ...
    CUDACHECK(cudaHostGetDevicePointer((void **)&state->hostBuffGpu, state->hostBuff, 0));
#endif
  }
  if (cpuPtr)
  ...
  if (gpuPtr)
  {
    if (cuda)
    {
      *gpuPtr = (sameProcess) ? state->cudaBuff : NULL;
    }
    else
    {
#ifdef _WIN32
      // On Windows, use the device pointer obtained from cudaHostGetDevicePointer
      *gpuPtr = (sameProcess) ? state->hostBuffGpu : NULL;
#else
      *gpuPtr = (sameProcess) ? state->hostBuff : NULL;
#endif
    }
  }
  if (ipcDesc)
...
static ncclResult_t sendProxyConnect(struct ncclProxyConnection *connection, struct ncclProxyState *proxyState, void *reqBuff, int reqSize, void *respBuff, int respSize, int *done)
{
  ...
  if (resources->shared)
  {
    ...
    if (resources->maxRecvs > 1 && ncclParamNetSharedComms())
    {
      ...
      if (comms->sendComm[resources->channelId] == NULL && comms->activeConnect[resources->channelId] == (resources->tpLocalRank + 1))
      {
  ...
  if (ret != ncclSuccess)
  {
    if (resources->netSendComm)
      proxyState->ncclNet->closeSend(resources->netSendComm);
    NCCLCHECK(ret);
  }
...
static ncclResult_t recvProxyConnect(struct ncclProxyConnection *connection, struct ncclProxyState *proxyState, void *reqBuff, int reqSize, void *respBuff, int respSize, int *done)
{
  ...
  if (resources->shared)
  {
    ...
    if (resources->maxRecvs > 1 && ncclParamNetSharedComms())
    {
      ...
      if (comms->recvComm[resources->channelId] == NULL && comms->activeAccept[resources->channelId] == (resources->tpLocalRank + 1))
      {
  ...
  *outRegBufFlag = 0;
  if (comm && userbuff && buffSize > 0 && nPeers > 0)
  {
    ...
    if (isValid)
    {
      ...
      if ((uint64_t)base + baseSize < (uint64_t)userbuff + buffSize)
        goto exit;
