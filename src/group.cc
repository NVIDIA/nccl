...
NCCL_THREAD_LOCAL int ncclGroupDepth = 0; // depth of ncclGroupStart nesting
NCCL_THREAD_LOCAL ncclResult_t ncclGroupError = ncclSuccess;
NCCL_THREAD_LOCAL struct ncclComm *ncclGroupCommHead[ncclGroupTaskTypeNum] = {nullptr};
NCCL_THREAD_LOCAL struct ncclComm *ncclGroupCommPreconnectHead = nullptr;
NCCL_THREAD_LOCAL struct ncclIntruQueue<struct ncclAsyncJob, &ncclAsyncJob::next> ncclAsyncJobs;
NCCL_THREAD_LOCAL int ncclGroupBlocking = -1; /* default mode */
...
ncclResult_t ncclP2PPreconnectFunc(struct ncclAsyncJob *job_)
{
  ...
  if (!job_->isThreadMain && CPU_COUNT(&comm->cpuAffinity))
