...
#pragma unroll
  for (int i = 0; i < n; i++)
    u[i] = __shfl_sync(-1u, u[i], root, nThreads);
  return v;
}
template <typename T>
...
#pragma unroll
  for (int i = 0; i < n; i++)
    u[i] = __shfl_sync(m, u[i], r);
  return v;
}

NCCL_DEVICE_INLINE nccl_ulong2_16 *ncclCoopBcast_WarpSpan_stash()
{
  __shared__ nccl_ulong2_16 stash[15];
  return stash;
}
...
NCCL_DEVICE_INLINE T ncclCoopBcast(ncclCoopWarpSpan coop, T value, int root, bool entrySync = true)
{
  ...
  if (entrySync)
    coop.sync();
  if (coop.thread_rank() == root)
    *(T *)&ncclCoopBcast_WarpSpan_stash()[coop.id] = value;
  coop.sync();
  return *(T *)&ncclCoopBcast_WarpSpan_stash()[coop.id];
}

NCCL_DEVICE_INLINE nccl_ulong2_16 *ncclCoopBcast_Cta_stash()
{
  __shared__ nccl_ulong2_16 stash;
  return &stash;
}
...
NCCL_DEVICE_INLINE T ncclCoopBcast(ncclCoopCta coop, T value, int root, bool entrySync = true)
{
  ...
  if (entrySync)
    coop.sync();
  if (coop.thread_rank() == root)
    *(T *)ncclCoopBcast_Cta_stash() = value;
  coop.sync();
