...
static ncclResult_t findLocalCpu(struct ncclTopoNode *node, struct ncclTopoNode **cpu, struct ncclTopoNode *from)
{
  *cpu = NULL;
  ...
  for (int l = 0; l < node->nlinks; l++)
  {
    ...
    if (node->links[l].type == LINK_PCI && node->links[l].remNode != from && (node->links[l].remNode->type == PCI || node->links[l].remNode->type == CPU))
    {
...
static ncclResult_t ncclTopoGetInterCpuBw(struct ncclTopoNode *cpu, float *bw)
{
  *bw = LOC_BW;
  ...
  {
    *bw =
        cpu->cpu.model == NCCL_TOPO_CPU_MODEL_INTEL_ERP ? ERP_QPI_BW : cpu->cpu.model == NCCL_TOPO_CPU_MODEL_INTEL_SRP ? SRP_QPI_BW
...
ncclResult_t ncclTopoConnectCpus(struct ncclTopoSystem *system)
{
  ...
  for (int n = 0; n < system->nodes[CPU].count; n++)
  {
    ...
    for (int p = 0; p < system->nodes[CPU].count; p++)
    {
      ...
      if (n == p || (NCCL_TOPO_ID_SYSTEM_ID(cpu1->id) != NCCL_TOPO_ID_SYSTEM_ID(cpu2->id)))
        continue;
      float bw;
...
ncclResult_t ncclTopoAddCpu(struct ncclXmlNode *xmlCpu, struct ncclTopoSystem *system)
{
  ...
  if (cpu->cpu.arch == NCCL_TOPO_CPU_ARCH_X86)
  {
    ...
    if (cpu->cpu.vendor == NCCL_TOPO_CPU_VENDOR_INTEL)
    {
      ...
      cpu->cpu.model =
          (familyId == 6 && modelId >= 0xCF) ? NCCL_TOPO_CPU_MODEL_INTEL_ERP : (familyId == 6 && modelId >= 0x8F) ? NCCL_TOPO_CPU_MODEL_INTEL_SRP
    ...
    else if (cpu->cpu.vendor == NCCL_TOPO_CPU_VENDOR_ZHAOXIN)
    {
      ...
      if (familyId == 7 && modelId == 0x5B)
        cpu->cpu.model = NCCL_TOPO_CPU_MODEL_YONGFENG;
    }
  }
  for (int s = 0; s < xmlCpu->nSubs; s++)
  {
    ...
    if (strcmp(node->name, "pci") == 0)
      NCCLCHECK(ncclTopoAddPci(node, system, cpu, systemId, numaId));
...
ncclResult_t ncclTopoAddPciLinks(struct ncclXmlNode *node, struct ncclTopoSystem *system, const char *parentBusId, int systemId)
{
  if (strcmp(node->name, "pcilink") == 0)
  {
    ...
    if (remote)
      NCCLCHECK(ncclTopoConnectNodes(pci, remote, LINK_LOC, LOC_BW));
  }
  else
  {
    if (strcmp(node->name, "cpu") == 0)
...
ncclResult_t ncclTopoGetSystemFromXml(struct ncclXml *xml, struct ncclTopoSystem **topoSystem, const uint64_t localHostHash)
{
  ...
  INFO(NCCL_GRAPH, "ncclTopoGetSystemFromXml: topNode has %d subs", topNode->nSubs);
  for (int s = 0; s < topNode->nSubs; s++)
  {
    struct ncclXmlNode *node = topNode->subs[s];
    INFO(NCCL_GRAPH, "ncclTopoGetSystemFromXml: sub[%d] name='%s'", s, node->name);
    if (strcmp(node->name, "cpu") == 0)
      NCCLCHECK(ncclTopoAddCpu(node, *topoSystem));
  }
  INFO(NCCL_GRAPH, "ncclTopoGetSystemFromXml: after parsing, nHosts=%d, localHostHash=0x%llx", system->nHosts, (unsigned long long)localHostHash);
  for (int i = 0; i < system->nHosts; i++)
  {
    INFO(NCCL_GRAPH, "ncclTopoGetSystemFromXml: hostHashes[%d] = 0x%llx", i, (unsigned long long)system->hostHashes[i]);
  }
...
ncclResult_t ncclFindFirstPciParent(ncclXmlNode **parent)
{
  ...
  while (strcmp(newParent->name, "pci") != 0)
  {
    ...
    if (newParent == nullptr)
      return ncclSuccess;
    if (strcmp(newParent->name, "system") == 0)
      return ncclSuccess;
  }
  *parent = newParent;
  return ncclSuccess;
...
ncclResult_t ncclTopoFindLinkWidthRec(ncclXmlNode *node, ncclXmlNode **physNetNodes, int ndevs, int *foundPhysNet, int *linkWidth)
{
  ...
  *foundPhysNet = 0;
  ...
  int devId = 0;
  while (devId < ndevs && !(*foundPhysNet))
    *foundPhysNet = (node == physNetNodes[devId++]);
...
static ncclResult_t ncclTopoPopulateNics(ncclXml *xml, int startIndex, int endIndex, struct ncclTopoNetInfo *netInfo, int virtualNics)
{
  for (int n = startIndex; n < endIndex; n++)
  {
    ...
    if (virtualNics)
    {
      ...
      if (net == NULL)
        NCCLCHECK(ncclTopoGetVNicParent(xml, netInfo->getProperties, &props.vProps, &parent));
    }
...
{
  if (forceMerge)
    *forceMerge = ncclGetEnv("NCCL_NET_FORCE_MERGE");
...
ncclResult_t ncclTopoGetNetDevsPolicy(enum netDevsPolicy *policy, int *policyNum)
{
  ...
  if (policy)
    *policy = netDevsPolicy;
  if (policyNum && netDevsPolicyNum >= 0)
    *policyNum = netDevsPolicyNum;
  return ncclSuccess;
}
...
ncclResult_t ncclTopoGetLocalNet(struct ncclTopoSystem *system, int rank, int channelId, int64_t *id, int *dev)
{
  ...
  if (dev)
    *dev = system->nodes[NET].nodes[localNets[net % localNetCount]].net.dev;
  return ncclSuccess;
}
