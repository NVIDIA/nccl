  ...
  if (got == -1u)
  {
    ...
    else
    {
      ...
      for (int k = 0; k < (int)ncclSymkKernelId_Count; k++)
      {
        ...
        {
          __atomic_store_n(&cache, (uint32_t)(1 << k), __ATOMIC_RELAXED);
          got = 1 << k;
          break;
        }
      }
    }
    __atomic_store_n(&cache, (uint32_t)got, __ATOMIC_RELAXED);
  }
  return got;
}
...
static void queryModel(struct ncclComm *comm, ncclSymkKernelId k, size_t nBytes, float *timeUs, int *nBlocks)
{
  ...
  if (nUserCTAs > 0)
    nMinBlocks = nMaxBlocks = nUserCTAs;
