...
#if __cplusplus
#if defined(_MSC_VER) && defined(NCCL_EXPORTS)
/* When building NCCL DLL on MSVC, extern "C" functions need dllexport for consistent linkage */
#define NCCL_EXTERN_C extern "C" __declspec(dllexport)
#elif defined(_MSC_VER)
/* When using NCCL DLL on MSVC, extern "C" functions need dllimport */
#define NCCL_EXTERN_C extern "C" __declspec(dllimport)
#else
...
// is useful to inhibit compiler optimizations.
    ...
    {
      return ord == cuda::memory_order_release ? cuda::memory_order_relaxed : ord == cuda::memory_order_acq_rel ? cuda::memory_order_acquire
                                                                                                                : ord;
    }
    NCCL_DEVICE_INLINE cuda::memory_order releaseOrderOf(cuda::memory_order ord)
    {
      return ord == cuda::memory_order_acquire ? cuda::memory_order_relaxed : ord == cuda::memory_order_acq_rel ? cuda::memory_order_release
                                                                                                                : ord;
    }
#endif
    ...
    NCCL_DEVICE_INLINE T loadConst(T const *p)
    {
      if (alignof(T) == 1)
      {
        ...
        for (int i = 0; i < (int)sizeof(T); i++)
          part[i] = __ldg((uint8_t const *)p + i);
        return ret;
      }
      else if (alignof(T) == 2)
      {
        ...
        for (int i = 0; i < (int)sizeof(T) / 2; i++)
          part[i] = __ldg((uint16_t const *)p + i);
        return ret;
      }
      else if (alignof(T) == 4)
      {
        ...
        for (int i = 0; i < (int)sizeof(T) / 4; i++)
          part[i] = __ldg((uint32_t const *)p + i);
        return ret;
      }
      else if (alignof(T) == 8)
      {
        ...
        for (int i = 0; i < (int)sizeof(T) / 8; i++)
          part[i] = __ldg((uint64_t const *)p + i);
        return ret;
      }
      else
      { // alignof(T) >= 16
        ...
        for (int i = 0; i < (int)sizeof(T) / 16; i++)
          part[i] = __ldg((ulonglong2 const *)p + i);
        return ret;
      }
    }
#endif
    ...
    struct Optional
    {
      ...
      NCCL_HOST_DEVICE_INLINE Optional(Present<Arg...> args, IntSeq<i...>) : present(true),
                                                                             thing{args.get(IntSeq<i>())...}
      {
      }
      // Construct with present thing:
      template <typename... Arg>
      NCCL_HOST_DEVICE_INLINE Optional(Present<Arg...> args) : Optional(args, IntSeqUpTo<sizeof...(Arg), 0>::Type())
      {
      }
