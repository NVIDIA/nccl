...
namespace {
  ...
  __device__ __forceinline__ void runRing(ncclWorkElem *args) {
    ...
    const ssize_t chunkSize = int(Proto::calcBytePerStep(args->ncclSteps)/sizeof(T) *
                                  (Proto::Id == NCCL_PROTO_SIMPLE ? REDUCESCATTER_CHUNKSTEPS : 1));
    ...
    Primitives<T, RedOp, FanSymmetric<1>, 0, Proto, 0>
      prims(tid, nthreads, &ring->prev, &ring->next, args->sendbuff, args->recvbuff, args->redOpArg, args->ncclSteps);
...
struct RunWorkElement<ncclFuncReduceScatter, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE> {
  __device__ __forceinline__ void run(ncclWorkElem *args) {
    using Proto = ProtoSimple<REDUCESCATTER_SLICE_PER_CHUNK, REDUCESCATTER_SLICESTEPS>;
